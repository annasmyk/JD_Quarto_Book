# X-11 decomposition {.unnumbered #m-X-11}

<<<<<<< HEAD
X-11 is the decomposition part of X13-Arima seasonal adjustment algorithm originally developed by the US Census Bureau. It is a non parametric algorithm based on [moving averages](#m-X-11-ma).
=======
X-11 is the decomposition part of X13-ARIMA seasonal adjustment algorithm originally developed by the US Census Bureau. It is a non parametric algorithm based on [moving averages](#m-x11-ma).
>>>>>>> 1a4297f10d537c24c1f7a2d238407c52058a2c4f

The genuine X-11 algorithm was meant to decompose monthly ($p=12$) and quarterly series ($p=4$). In JDemetra+ half-yearly data ($p=2$), quadri-monthly ($p=3$) and bi-monthly data ($p=6$) can also be handled. ($p=3$ and $p=6$ only in version 3.x)
...
In recent years, X-11 implementation in JDemetra+ v 3.x has been extended to infra-monthly data (weekly, daily, hourly...). Handling this kind of data gave way to a tailored X-11 algorithm whose peculiarities are described in [this chapter](#m-sa-hf).




## In this chapter {#m-X-11-intro}

This chapter provides details on 
    - [algorithm steps](#X-11-steps)
    - [computation stages and detailed output series](#m-X-11-stages-output)
    - [quality measures](#m-X-11-qm)
    - [filter length choices](#m-X-11-filters-length-choice)
    - [extreme values correction](#X-11-I-correc)

The practical implementation as well as all the options, using the graphical user interface or R packages are described in [this chapter](#sa-X-11)

ref to add: the X-11 method

## Moving averages in X-11 {#m-X-11-ma}

Moving averages (MA) are the building blocks of X-11. They will be used successively to accomplish three goals:
...
## Processing stages and output {#m-X-11-stages-output}
...
![](All_images/ESTP_bbk_X-11_basic_principle.png)

![](All_images/ESTP_bbk_X-11_detailed_computation.png)
...
-   $M7$ is the combined test for the presence of an identifiable
    ...
    and moving seasonality[^m-X-11-decomposition-2].
...
#### Moving seasonality ratios (MSR) {#m-X-11-msr}
...
The Moving Seasonality Ratio (MSR) is used to measure the amount of noise in the Seasonal-Irregular component. By studying these values, the user can [select for each period the seasonal filter](#a-X-11-seaso-filters) that is the most suitable given the noisiness of the series.

![Text](All_images/RDimage56.png)

## Filter length choice {#m-X-11-filters-length-choice}

### Trend estimation with Henderson Moving average {#m-X-11-trend-filters}
...
![Text](All_images/ESTP_bbk_X-11_Henderson_choice_formula.png)


### Seasonality extraction filters {#m-X-11-seaso-filters}
...
These ratios are presented in [Detailed Quality Measures](#m-X-11-msr)
...
Though it is recommended rely on the pre-adjustment stage to correct for outliers (transparent method with explicit modelling), X-11 has its own (historical) module for identification and treatment of extreme values based on a comparison between the actual and the theoretical value of $I$. 
