...
If a [pre-treatment](#sa-pre-t) is performed using X-13-Arima algorithm is A step is not used, results in A tables appearing in JDemetra+ output are a copy of pre-treatment results. 

### Stage B: First automatic correction of the series

This stage consists of a first estimation and [down-weighting of
extreme observations](#X-11_I-correc). This stage is performed by applying twice the algorithm [steps](#X-11-steps) outlined above. It starts with the linearised or raw series copied in table B1 lead to table B20, containing adjustment values for extreme observations. B1 corrected with weights from B20 allows to compute the series C1 which will start the next stage.
...
### Part D: Final decomposition and seasonal adjustment
...
Including pre-adjustement effects (check versions, quick test)
...
## Extreme values: identification and replacement {#X-11_I-correc}
