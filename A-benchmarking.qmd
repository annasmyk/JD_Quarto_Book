...
These methods provide a high-frequency series (input series) modified so that it fulfills a linear relation, with another of low frequency (benchmark), both series measure the same target variable. An example of the relation to be fulfilled could be that the
...
Let see a description of the available options on Cholette method:
...
3. **Rho**: value between $-1$ and $1$. It is the coefficient of an AR($1$) model that follows the error term. The default value is $1$, equivalent to a apply Denton.

4. **Lambda**: value between $0$ and $1$. It is the parameter $\lambda$ of the following fucntion to be minimize in Cholette method:
    $$
...
In both cases, Denton and Cholette methods, the output is a graph with the original series and the bechmarked series. There is no table with the results, but it is very easy to create one from the graph. Do click on the graph and select copy option, then paste the values in excel (control-V).
...
These methods are used to disaggregate a series from low frequency to high-frequency. Temporal disaggregaton methods, when there is a high frequency related indicators, provide estimations for a series, applying a regression model that relates low frequency series observed (input series) with p auxiliar series (proxy). This p auxiliar series are available on a high frequency basis.
It is assumed that the high frequency observations of the series to be estimated satisfy a multiple regression relationship with p related series.
