...
In both cases, Denton and Cholette, the output is a graph with the original series and the bechmarked series. There is no table with the results, but it is very easy to create one from the graph. Do click on the graph and select copy option, then paste the values in excel (control-V).
