...
Using the [GUI](#t-gui-overview) with a [plug-in](#t-plug-ins) or [rjd3bench package](#t-r-packs) package.
...
| Method              | GUI Plug-in for V 2.x | GUI Plug-in for V 2.x | In R rjd3bench |
|---------------------|-----------------------|-----------------------|----------------|
| Regression Models\* | ✔️                    | ✔️                    | ✔              |
| Model-based Denton  | ✖                     | ✔                     | rjd3sts        |

\*Regression models: several structure of residuals
...
4.  Both pictures and the table can be copied the usual way (see the [*Simple seasonal adjustment of a single time series*](../case-studies/simplesa-single.html) scenario).
...
6.  Expand the "+" menu and choose an appropriate data format (here Excel has been chosen). It is possible to save the results in TXT, XLS, CSV, and CSV matrix formats. Note that the [available content of the output depends on the output type](../theory/output.html).

    ![**Exporting data to an Excel file**](All_images/UG_SSA_image29.jpg)

7.  Chose the output items that refer to the results from the benchmarking procedure, move them to the window on the right and click **OK**.

    ![**Exporting the results of the benchmarking procedure**](All_images/UDimage4.jpg)

### In R with `rjd3x13` and `rjd3tramoseats`

When performing seasonal adjustment with `rjd3x13` and `rjd3tramoseats`, the current (or default) specification has to be customized using the function `rjd3toolkit::set_benchmarking` documented on this [GitHub page](https://rjdemetra.github.io/rjd3toolkit/reference/set_benchmarking.html)
...
More information on R packages for JDemetra+ and installation procedures is provided in [this chapter](#t-r-packs)
...
Download the plug-in for GUI as explained [here](#t-plug-ins-bench) and install it as detailed [here](#t-plug-ins-inst)
...
The plug-in for v 3.x is very similar to the one in v2.x described above. Some methods are not available yet in v 3.x, which contains Model Based Denton not included in v2.x.

Up coming content.

### In R with `rjd3bench`

Use the \[rjd3bench\](https://github.com/rjdemetra/rjd3bench) package and see its documentation pages. Browse its documentation on this [GitHub page](https://rjdemetra.github.io/rjd3bench/).

To get started browse the [vignette](https://rjdemetra.github.io/rjd3bench/articles/rjd3bench.html)
...
Temporal disaggregation in the GUI is available with the same [plug-in](#a-bench-td-plugin) as benchmarking (described in the sections above)
