# SA: X-11 Decomposition {#sa-x11 .unnumbered}

## In this chapter

This chapter focuses on practical implementation of an X-11 decomposition using the graphical user interface [GUI](#tools-GUI) and R using [R packages](#t-r-packs) in version 2.x and 3.x. More explanations on X-11 algorithm can be found [here](#m-x11).
...
| Version  | GUI                | R                  |
|----------|--------------------|--------------------|
| v 2.x    | $p=12, 4, 2$       | $p=12, 4, 2$       |
| v 3.x    | $p=12, 6, 4, 3, 2$ | $p=12, 6, 4, 3, 2$ |

## Default specifications {#a-x11-default_specs}
...
The model_sa_v2 R object (list of lists) contains all parameters and results. Its structure is detailed [here](#t-r-packs-rjdv2-structure). 
...
Full documentation of 'rjd3x13::x13' function can be found [here](https://rjdemetra.github.io/rjd3x13/reference/x13.html) and of 'rjd3x13::x11'
[here](https://rjdemetra.github.io/rjd3x13/reference/x11.html).
...
[Detailed results](#m-x11-stages-output) from decomposition are displayed in Decomposition (X-11) node.
...
For each period (month, quarter) the final value of the seasonal factors (without calendar factors, Table D10) is plotted (blue line). The dots represent $S+I$ or $S*I$ in the multiplicative case, where I The red straight line is the average of the factors over the decomposition (estimation) span.

In GUI all values cannot be retrieved.

(to be checked: I=D8 or I = D13 ?)
(D8 doesn't contain pre-treatment effects and D13 does)

##### Retrieve in R

All the values and the same plot as descibed above can be generated in R, the span can be customized. 
