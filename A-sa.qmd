...
The default specifications for X-11 must be chosen at the starting of the SA processing. They are detailed in the [RegArima part](A-sa.qmd#Pre-treatment:%20Reg-ARIMA%20(or%20TRAMO))
...
Output series can be exported out if GUI by two means:
- generating output files from GUI as explained here 
- running the cruncher to genereate those files as described here 
...
To display results in GUI, expande NODE 
Decompostion(X-11) \> Quality Measures \> Summary
...
in GUI all the diagnostics below can be retrieved expandin the NODE 
...
(here clarify notations)
(link to tables details in meth chapter or appendix)
(here A, D, E are needed)
(link to Ladiray-Quenneville)
...
absolute changes over several periods are calculated as:
...
For the multiplicative decomposition the following formula is
    used: 
...
Global IC is displayed below the table
...
component ($I$), trend made stationary ($C$), seasonal
component ($S$) and calendar effects (TD&H). 
...
The [Moving Seasonality Ratio (MSR)](M-X11-decomposition.qmd) is used to measure the amount of noise in the Seasonal-Irregular component. By studying these values, the
user can [select for each period the seasonal filter](../reference-manual/sa-spec-X13.html#seasonal-filter-x11-seasonalma) that is the most suitable given the noisiness of the series.
