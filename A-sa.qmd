...
Need 2: taking national calendar into account 
Solutions 
...
ATT: additional options in Tramo
...
The following section details how to retrieve results (parameters, regressors, regression coefficients and test) when using GUI or R interface.

##### Parameters

Parameters are regressors used in fine.
If non test options, parameters are known 
If test options are selected by the algorithm.
...
Residual calendar effects are tested with A F-Test 7 regressors and no national calendar, on sa final series and on irregular component
(link to calendar chapter for test details)

**In GUI**

F-Test results are displayed at hte bottom of **Main  Results** NODE in the SAProcessing panel
...
(details will go to calendar chapter)
...
overview: 
why use outliers and what happens (effects reallocated)
in depth method outline in outliers chap 
what is available 
...
#### Options for pre-specified outliers 
   ...
   User-defined outliers are used when prior knowledge suggests that certain effects exist at known time points[^14]. 
   Four pre-defined outlier types, which are simple forms of intervention variables, are implemented:
   * Additive Outlier (AO);
   * Level shift (LS); 
...
In main results NODE
(same info at top of pre-processing NODE)
...
(some kind of summary chapter introduction)
- rationale

- common parameters: assign to a component

rationale and equations (equalities)

Pre-treatment regression with additionnal outliers 
...
(att some are missing)
$reg= reg_{i}+reg_{t}+reg_{s}+...$
	The user-defined regression variable associated to a specific component should not contain effects that have to 
	be associated with another component. Therefore, the following rules should be observed: 
    ...
    * The variable assigned to the irregular should contain neither a seasonal pattern nor a trend (or level).
    - no external regressors can be assigned to calendar component. It has to be be done via user defined calendar regressors specific part (link)
...
see if here: ?
The graph and formula are available in [Linearisation with the TRAMO and RegARIMA models](../theory/SA_lin.html).
...
###### Allocation to components 
allocation when intervention or ramps ?
in test allocated to trend ? (reg)
...
Intervention variables are modeled as any possible sequence of ones and zeros, on which some operators may be applied. Intervention variables are built as combinations of the following basic structures[^16]: 
   *  Dummy variables[^17];
...
step 3: 
![](All_images/SA_REGA_calendar_user_def_inter_var_3.png)
explain params 
- differentiation orders
D1DS : if ticked: 1 1 automatically 
(pb differentiation before the real differentiation ??)
...
Overview: 
differences 
GUI set up vs R set up  


#### Retrieving Results 
(for all types of external regressors: user def imported, ramps or intervention variables)

##### Regressors

**In GUI**

To retrieve regressors a actually used, expand pre-processing NODE and click on Regressors pane.
...
(for both options) *fcst.horizon* the forecasting horizon (numeric). The forecast length generated by the Reg-Arima model in periods (positive values) or years (negative values). 
By default, the program generates a two-year forecast (fcst.horizon set to -2).
Defaults different in GUI and  R.


**Setting in GUI**
![](All_images/SA_REGA_arima_auto_params.png)


Forecast horizon when using tramo seats 
Is set in the decomposition part of the specification in GUI.

![](All_images/SA_REGA_arima_forecast_horizon_tramo.png)


Forecast horizon when using X13-ARIMA 
![](All_images/SA_REGA_arima_forecast_horizon_X11.png)

**Setting in R**
(first template, then worked example)
...
**Setting in GUI**
![](All_images/SA_REGA_Arima_user_params_1.png)

Fixing coefficients
![](All_images/SA_REGA_Arima_user_params_2.png)
...
Type of results (yc Tramo addenda)
- all regressors used (shown above)
- regression details: explanatory variables (above) and Arima model specific results 
- additional diagnostics on residuals
- likelihood
- seasonality tests on residuals
(Highlight differences: 
- in gui 
- in R v2: retrieve object in output, v3 run test)


#### Display in GUI 

Reg-Arima model detail with other regression results in pre-processing pane.
with number of observations..
parameters 
![](All_images/SA_REGA_Arima_model_results_1.png)
...
SEATS (Signal Extraction for Arima Time Series) algorithm will decompose the linearized series, in level or in logarithm, using the Arima model fitted by Tramo (link) in the pre-treatment phase.
...
Generating output from GUI (link) or from Cruncher (link), component series, their standard errors, forecasts and forecasts errors can be acessed with the following names
