...
(details will go to calendar chapter) (where are the links leading here: - - )
...
overview: why use outliers and what happens (effects reallocated) in depth method outline in outliers chap what is available

automatic detection The *Outliers* section enables the user to perform an automatic detection of additive outliers, temporary change outliers, level shifts, seasonal outliers, or any combination of these four basic outlier types, using the specified model. pre-specified
...
(some kind of summary chapter introduction) - rationale
...
Step 3: Modify specifications via window Modifications are done the same way in a global specification (whole SAP) or series by series.
...
step 3: ![](All_images/SA_REGA_calendar_user_def_inter_var_3.png) explain params - differentiation orders D1DS : if ticked: 1 1 automatically (pb differentiation before the real differentiation ??)
...
Type of results (yc Tramo addenda) - all regressors used (shown above) - regression details: explanatory variables (above) and Arima model specific results - additional diagnostics on residuals - likelihood - seasonality tests on residuals (Highlight differences: - in gui - in R v2: retrieve object in output, v3 run test)
...
Details on residuals ![](All_images/SA_REGA_Arima_model_results_2.png)
...
global details details on diagnostics
...
This section describes the parameters which are automatically chosen by the software as a result of the estimation process. They have no default value. All parameters having a default value and beign
...
SEATS (link to method chapter) algorithm will decompose the linearized series, in level or in logarithm, using the Arima model fitted by Tramo (link) in the pre-treatment phase.
