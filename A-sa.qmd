...
(generic sentence to quasi repeat at the end of each part) Once regressors set, the reg arima (tramo) model will be estimated gloablly with all the other regression variable and taking into account arima model specificities as well. That is why diagnostics are all jointly displayed at the end of the process. (link)
...
Regression Results - tables with results for all variables (auto and user-defined) student, F test - tests

Arima model results

Goodness of fit Test on Arima model residuals list retrieve in GUI retireve in R

Residual calendar effects what kinf of test is performed ? td spectral peaks - residual effect in $SA$ or $I$ series
