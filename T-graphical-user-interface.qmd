...
By default, on the left hand side of the window two panels are visible: the [*Workspace* panel](#workspace-window) and the [*Providers* panel](#providers-window). The *Workspace* panel stores the work performed by the user in a coherent and structured way. The *Providers* panel presents the list of the [data sources](#providers-window) and organizes the imported series within each data provider. By default, JDemetra+ supports the following data sources: 
...
- The [Providers](#providers-window) window, which organises time series; 

- The [Workspace](#workspace-window) window, which stores results generated by the software as well as settings used to create them; 
...
The *File* menu is intended for working with [workspaces](#workspace-window) and [data sources](../reference-manual/data-providers.html). 
