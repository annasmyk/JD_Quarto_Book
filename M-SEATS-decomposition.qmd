...
Seats is an ARIMA Model Based (AMB) decomposition method which is the second part of Tramo-Seats seasonal adjustment algorithm, originally developed by the Bank of Spain [@caporello2004program].

Seats draws from the ARIMA-model-based (AMB) decomposition framework developed by CLEVELAND, W.P., and TIAO, G.C. [-@cleveland1976decomposition], BURMAN, J.P. [-@Burman1980], HILLMER, S.C., and TIAO, G.C. [-@HillmerTiao1982], BELL, W.R., and HILLMER, S.C. [-@BellHillmer1984] and MARAVALL, A., and PIERCE, D.A. [-@maravall1987prototypical].


<!-- References: -->
<!-- A methodological overview of the entire process can be found in  -->
<!-- - maravall 1995: cannot find ref  -->
<!-- - planas book  -->
<!-- KEY: explanation holes vs planas or Gomez ? -->

<!-- should attach the 2 pdfs to quarto book ? -->
...
## Seats steps: overview {#m-seats-steps}

Seats decomposes the linearized series into trend, seasonal, transitory and irregular components, provides forecasts for these components, together with the associated standard errors, and finally assigns the deterministic effects to each component yielding the final components.

## ARIMA model identification for the components

### Input from Tramo

In JDemetra+ the input for the model based signal extraction procedure is always provided by Tramo and includes the original series $y_{t}$, the linearized series $x_{t}$ (i.e. the original series $y_{t}$ with the deterministic effects removed), the ARIMA model for the linearized time series $x_{t}$ and the deterministic effects (calendar, outliers and other regression variable effects).

### ARIMA modelling of the raw series

One of the fundamental assumptions made by Seats is that the linearized time series $x_{t}$ follows an ARIMA model:

$$
\phi(B)\delta(B)x_{t} = \Theta(B)a_{x,t}
$$ {#eq-1}

where:

-   $B$ -- backshift operator $B(x_{t}) = x_{t - 1}$;

-   $s$ -- number of observations per year

-   $\delta(B)$ -- a non-stationary autoregressive (AR) polynomial in $B$ and $B^{S}$ (regular and seasonal unit roots);

-   $\Theta(B)$ -- an invertible moving average (MA) polynomial in $B$ and in $B^{S}$, which can be expressed in the multiplicative form $(1 + \vartheta_{1}B + \ldots{+ \ \vartheta_{q}}B^{q})(1 + \theta_{1}B^{s} + \ldots{+ \ \theta}_{Q}B^{\text{sQ}})$ ;

-   $phi(B)$ -- a stationary autoregressive (AR) polynomial in $B$ and in $B^{S}$ 

-   $a_{x,t}$ -- a white-noise variable with the variance $V_{a}$.

It should be noted that the time series $x_{t}$ can be predicted using its past observations and making an error. The variable $a_{x,t}$, which is assumed to be white noise, is the fundamental *innovation* to the series at time *t*, that is the part that cannot be predicted based on the past history of the series.

Denoting $\Phi(B) = \phi(B)\delta(B)$,can be written in a more concise form as

$$
\Phi(B)x_{t} = \Theta(B)a_{x,t}
$$ {#eq-2}

where $\Phi(B)$ contains both the stationary and the nonstationary (unit) roots.

### Derivation of the models for the components

Let us consider an additive decomposition model(if a multiplicative model is better suited, the series can be log-transformed)

$$
x_{t} = \sum_{i = 1}^{k}x_{\text{it}}
$$ {#eq-3}

where *i* refers to the components: trend, seasonal, transitory and irregular. These components are by assumption orthogonal. Apart from the irregular component, supposed to be a white noise, 

A helpful simple case is to consider the series as a sum of a signal $s_{t}$ (of interest) and noise $n_{t}$
$$
x_{t} = s_{t} + n_{t}
$$ 

it is assumed that each component follows an ARIMA model which can be represented, using the notation of @eq-2 as:

$$
\Phi_{i}(B)x_{\text{it}} = \Theta_{i}(B)a_{\text{it}}
$$ {#eq-4}

where

-   $\Phi_{i}(B) = \phi_{i}(B)\delta_{i}(B)$,

-   $\Phi_{i}(B)$ and $\Theta_{i}(B)$ are finite polynomials of order $p_{i}$ and $q_{i}$, respectively,

-   $a_{\text{it}}$ is a white noise process with zero mean and constant variance $V_{ai}$ and $a_{\text{it}}$ and $a_{\text{jt}}$ are not correlated for $i \neq j$ and for any $t$.

In the simplest case, we would have two models : 

- model for the signal

$$
\Phi_{s}(B)s_{t} = \theta_{s}(B)a_{s,t}
$$ 

- model for the noise 

$$
\Phi_{n}(B)n_{t} = \theta_{n}(B)a_{n,t}
$$

These disturbances are functions of the innovations in the series and are called "pseudo-innovations" in the literature concerning the AMB decomposition as they refer to the components that are never observed, in JDemetra+ documentation the term "innovations" is used to refer to "pseudo-innovations".(GÓMEZ, V., and MARAVALL, A. (2001a).

<!-- make the paragraph below more readable  -->

The following assumptions hold for @eq-4. For each $\text{i}$ the polynomials $\phi_{i}(B)$, $\delta_{i}(B)$ and $\Theta_{i}(B)$ are prime and of finite order. The roots of $\delta_{i}(B)$ lies on the unit circle; those of $\phi_{i}(B)$ lie outside, while all the roots of $\Theta_{i}(B)$ are on or outside the unit circle. This means that nonstationary and noninvertible components are allowed. Since different roots of the AR polynomial induce peaks in the spectrum of the series at different frequencies, and given that different components are associated with the spectral peaks for different frequencies, it is assumed that for $i \neq j$ the polynomials$\ \phi_{i}(B)$ and $\phi_{j}(B)$ do not share any common root (they are coprime). Finally, it is assumed that the polynomials $\Theta_{i}(B),\ i = 1,\ldots,k$ are prime and share no unit root in common, guaranteeing the invertibility of the overall series. In fact, since a unit root in $\Theta_{i}(B)$ induces a spectral zero, when the polynomials $\Theta_{i}(B),\ i = 1,\ldots,k$ share no unit root in common, there is no frequency for which all component spectra become zero.

For description of the spectrum see chapter on [Spectral Analysis](#m-spectrum).(MARAVALL, A. (1995).

By replacing the series $x_t$ and its components by their models in (@eq-3), the following identity can be derived :

$$
\frac{\Theta(B)}{\Phi(B)}a_{x,t} = \sum_{i = 1}^{k}{\frac{\Theta_{i}(B)}{\Phi_{i}(B)}a_{\text{it}}}
$$ {#eq-5}

By polynomial identification in @eq5, the two following identities can be derived: 

- a relation between the series and components AR polynomials

$$
\Phi(B) = \prod_{i = 1}^{k}{\Phi_{i}(B)}
$$ {#eq-6}

In the simplest case 
$$
\Phi(B) = \Phi_{s}(B)\Phi_{n}(B)
$$
- a relation between the series MA polynomial and components AR and MA polynomials


$$
\Theta(B)a_{x,t} = \sum_{i= 1}^{k}{\Phi_{\text{ni}}(B)}\Theta_{i}(B)a_{\text{it}}
$$ {#eq-7}

where 

- $\Phi_{\text{ni}}(B)$ is the product of all $\varphi_{j}(B),\ j = 1,\ldots,k$, except $\Phi_{i}(B)$. 

In the simplest case 

$$
\Theta(B)a_{x,t} = \Phi_{s}(B)\theta_{n}(B)a_{n,t}+\Phi_{n}(B)\theta_{s}(B)a_{s,t}
$$
...
Consider AR polynomial $\Phi(z)$. It has real and complex roots (solutions). Complex number $x = a + bi$, with $a$ and $\text{b}$ both real numbers, can be represented as $x = r(cos(\omega) + i\ sin(\omega))$, where $i$ is the imaginary unit ${i}^{2}=-1)$, $r$ is the modulus of $x$, that is $r=|x| = \sqrt{a^{2} + b^{2}}$ and $\omega$ is the argument (frequency). When roots are complex, they are always in pairs of complex conjugates. The representation of the complex number $x = a + bi$ has a geometric interpretation in the complex plane established by the real axis and the orthogonal imaginary axis.
...
Under identification problem to be explained 

- parameters to estimate 

- equations

Further assumptions are therefore needed to cope with the under identification problem: 

i) $p_{i} \geq q_{i}$ 

ii) the canonical decomposition, i.e. the decomposition that allocate all additive white noise to the irregular component (yielding non invertible components except the irregular).
...
From @eq-6 and @eq-7 it is clear that $\varphi(B) = \varphi_{s}(B)\varphi_{n}(B)$ and $\theta(B)a_{x,t} = \theta_{s}(B)a_{\text{st}}+\theta_{n}(B)a_{\text{nt}}$.
...
-   $k_{s}=\frac{V(a_{s})}{V_{a}}$,

$V(a_{s})$ is the variance of $a_{st}$ and $V_{a}$ is the variance of $a_{x,t}$.
...
$$
acgf(B) = \frac{\theta(B)\theta(F)}{\phi(B)\delta(B)\phi(F)\delta(F)}V_{a}
$$ {#eq-26}
...
-   $\gamma(B,F) = \frac{\theta(B)\theta(F)}{\phi(B)\delta(B)\phi(F)\delta(F)}V_{a}$ is the p-ACGF of $x_{t}$.
...
$$
\widehat{x}_{it|t} = E_{t}x_{it}=E_{t}\widehat{x}_{it} = {\xi}_{i}(B)^{-}a_{x,t}
$$ {#eq-35}
