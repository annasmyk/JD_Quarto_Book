...
the linearized series $x_{t}$ (i.e. the original series $y\_{t}$ with
...
effects)[^m-seats-decomposition-1]. SEATS decomposes the linearized
...
components[^m-seats-decomposition-2]. The Minimum Mean Square Error
(MMSE) estimators of the components are computed with a
Wiener-Kolmogorov filter applied to the finite series extended with
forecasts and backcasts[^m-seats-decomposition-3].

[^m-seats-decomposition-1]: In the original software SEATS can be used
...
-   $\phi(B)$ -- a stationary autoregressive (AR) polynomial in $B$ and
    in $B\^{S}$ containing regular and seasonal unit roots, with *s*
...
@eq-1 <!---\[7.25\]--> can be written in a more concise form as
...
model which can be represented, using the notation of @eq-2
<!---\[7.26\]-->, as:
...
-   $\varphi_{i}\left( B \right) = \phi_{i}\left( B \right)\delta_{i}\left( B \right),\ \ x_{\text{it}}$
    is the *i*-*th* unobserved component,

-   $\varphi_{i}\left( B \right)$ and $\theta_{i}\left( B \right)$ are
    finite polynomials of order $p_{i}$ and $q_{i}$, respectively,

-   $a_{\text{it}}$, the disturbance associated with such component, is
    a white noise process with zero mean and constant variance
    $V(a_{i})$ and $a_{\text{it}}$ and $a_{\text{jt}}$ are not
    correlated for $i \neq j$ and for any $t$..
...
[^m-seats-decomposition-4]. In the JDemetra+ documentation the term
"innovations" is used to refer to the "pseudo-innovations".

[^m-seats-decomposition-4]: GÓMEZ, V., and MARAVALL, A. (2001a).

The following assumptions hold for @eq-4 <!---\[7.28\]-->. For each
...
spectrum[^m-seats-decomposition-5] of the series at different
...
zero[^m-seats-decomposition-6].

[^m-seats-decomposition-5]: For description of the spectrum see section
...
$x\_{t}$ will also follow an ARIMA model, as in @eq-2 <!---\[7.26\]-->,
...
of a root will be explored[^m-seats-decomposition-7].

[^m-seats-decomposition-7]: Description based on KAISER, R., and
    MARAVALL, A. (2000) and MARAVALL, A. (2008c).

The equation @eq-2 <!---\[7.26\]--> can be expressed as:

$$
\psi^{- 1}(B)x_{t} = a_{t}(1 + \varphi_{1}B + \ldots\varphi_{p}B^{p})x_{t} =(1 + \theta_{1}B + \ldots\theta_{q}B^{q})a_{t}
$$ {#eq-8} <!---\[7.32\]-->

Let us now consider @eq-2 <!---\[7.26\]--> in the inverted form:

$$
\theta\left( B \right)y_{t} = \varphi(B)a_{t}
$$ {#eq-9} <!---\[7.33\]-->

If both sides of @eq-8 <!---\[7.32\]--> are multiplied by $x_{t - k}$
...
The autocorrelation function $\gamma_{k}$ is a solution of @eq-10
...
If $z_{1}$,...,$\ z_{p}$ are the roots of @eq-11 <!---\[7.35\]-->, the
solutions of @eq-10 <!---\[7.34\]--> can be expressed as:
...
$\left| r_{i} \right| < 1,\ i = 1,\ldots,p$. From @eq-10
<!---\[7.34\]--> and @eq-12 <!---\[7.36\]--> it can be noticed that
...
Equation @eq-11 <!---\[7.35\]--> has real and complex roots (solutions).
...
![**Geometric representation of a complex number and of its
conjugate**](All_images/UG_A_image3.png)

Representing the roots of the characteristic equation @eq-11
...
derived. The period of function @eq-14 <!---\[7.38\]-->, denoted by
...
and $\pi$. The corresponding number of cycles per year and the length of
the movements are presented in the table below:
...
i.e.:[^m-seats-decomposition-8]

[^m-seats-decomposition-8]: For details see MARAVALL, A., CAPORELLO, G.,
    PÉREZ, D., and LÓPEZ, R. (2014).

-   Roots of $\left( 1 - B \right)^{d}$ are assigned to trend component.

-   Roots of
    $\ \left( 1 - B^{s} \right)^{d_{s}} = {((1 - B)(1 + B + \ldots + B^{s - 1}))}^{d_{s}}$
    are assigned to the trend component (root of
    ${\left( 1 - B \right)}\^{d\_{s}}$) and to the seasonal component
    (roots of ${(1 + B + \ldots + B\^{s - 1})}\^{d\_{s}}$).
...
-   When the modulus of the inverse of a real positive root of
    ...
    original SEATS it is controlled by
    *rmod*)[^m-seats-decomposition-9], then the root is assigned to the
    trend component. Otherwise it is assigned to the transitory
    component.

-   Real negative inverse roots of $\text{ϕ}_{p}\left( B \right)$
    associated with the seasonal two-period cycle are assigned to the
    seasonal component if their modulus is greater than *k*, where $k$
    is the threshold value controlled by the *Seasonal boundary* and the
    *Seas. boundary (unique)* parameters. Otherwise they are assigned to
...
[^m-seats-decomposition-9]: In JDemetra+ this argument is called *Trend
    boundary*.
...
presented by @eq-2 <!---\[7.26\]--> yields to the identification of the
...
the transitory component.[^m-seats-decomposition-10]

[^m-seats-decomposition-10]: The AR roots close to or at the trading day
...
pseudo-spectrum[^m-seats-decomposition-11] of $x_{t}$ into additive
...
decomposition is called admissible[^m-seats-decomposition-12]. In such
...
of S.C. Hillmer and G.C. Tiao is applied[^m-seats-decomposition-13],
i.e. all additive white noise is added to the irregular component As a
...
noninvertible[^m-seats-decomposition-14]. Given the stochastic features
of the series, it can be shown by that the canonical decomposition
produces as stable as possible trend and seasonal components since it
maximizes the variance of the irregular and minimizes the variance of
the other components[^m-seats-decomposition-15]. However, there is a
price to be paid as canonical components can produce larger revisions in
the preliminary estimators of the component[^m-seats-decomposition-16]
than any other admissible decomposition.

[^m-seats-decomposition-11]: The term pseudo-spectrum is used for a
    non-stationary time series, while the term spectrum is used for a
    stationary time series.

[^m-seats-decomposition-12]: If the ARIMA model estimated in TRAMO does
...
![**A comparison of canonical trend and admissible
trend**](All_images/UG_A_image5.png)
...
A pseudo-spectrum for a monthly time series $x\_{t}$ is presented in the
...
and bounded[^m-seats-decomposition-17]. The frequencies in the range
...
[^m-seats-decomposition-18]

[^m-seats-decomposition-17]: KAISER, R., and MARAVALL, A. (2000).

[^m-seats-decomposition-18]: KAISER, R., and MARAVALL, A. (2000).

![**The pseudo-spectrum for a monthly
series**](All_images/UG_A_image6.png)
...
*MCElroyMatrix*[^m-seats-decomposition-19]. Here the first of
abovementioned options, proposed by BURMAN, J.P. (1980) will be
explained.

[^m-seats-decomposition-19]: The choice of the estimation method is
...
model @eq-2 <!---\[7.26\]--> and from @eq-4 <!---\[7.28\]--> the models
...
From @eq-6 <!---\[7.30\]--> and @eq-7 <!---\[7.31\]--> it is clear that
...
$X_{T}$.[^m-seats-decomposition-20] WHITTLE (1963) shows that the MMSE
estimator of ${\widehat{s}}_{t}$ is:

[^m-seats-decomposition-20]: MARAVALL, A. (2008c).
...
$V(a_{s})$ is the variance of $a_{st}$ and $V(a)$ is the variance of
$a_{t}$.

Expressing the $\psi\left(B\right)$ polynomials as functions of the AR
and MA polynomials, after cancelation of roots, the estimator of $s_{t}$
can be expressed as:
...
Equation @eq-24 <!---\[7.48\]--> shows that the WK filter is two-sided
...
The WK filter from @eq-24 <!---\[7.48\]--> can also be expressed as a
...
From @eq-24 <!---\[7.48\]--> it can be seen that the WK filter depends
...
seasonality[^m-seats-decomposition-21]. Examples of WK filters for
stochastic and stable seasonal components are presented on the figure
below.

[^m-seats-decomposition-21]: MARAVALL, A. (1995).

![**WK filters for stable and stochastic seasonal
components**](All_images/UG_A_image9.png)
...
for large $k$ the estimator for the central periods of the series can be
safely seen as generated by the WK filter[^m-seats-decomposition-22]:

[^m-seats-decomposition-22]: MARAVALL, A., and PLANAS, C. (1999).
...
estimator expressed by @eq-23 <!---\[7.47\]--> can be assumed as the
final (historical) estimator for the central observations of the
series[^m-seats-decomposition-23]. In practice, the Wiener-Kolmogorov
...
[^m-seats-decomposition-23]: MARAVALL, A. (1998).
...
the gain of the filter.[^m-seats-decomposition-24] GÓMEZ, V., and
...
[^m-seats-decomposition-24]: GÓMEZ, V., and MARAVALL, A. (2001a).
...
-   $\ \left\lbrack \widetilde{\nu}\left( \omega \right) \right\rbrack^{2} = \left\lbrack \frac{g_{s}(\omega)}{g_{x}(\omega)} \right\rbrack^{2} = \left\lbrack \frac{g_{s}(\omega)}{g_{s}(\omega) + g_{n}(\omega)} \right\rbrack^{2} = \left\lbrack \frac{1}{1 + \frac{1}{r(\omega)}} \right\rbrack^{2}$
    is the squared gain of the filter ;
...
![**Component spectrum and estimator spectrum for
trend**](All_images/UG_A_image10.png)

From the equation @eq-29 <!---\[7.53\]--> it is clear that the squared
...
${\widehat{s}}_{t}$. These two cases are well illustrated by the figure
below that shows the square gain of the WK filter for two series already
analysed in the figure above (Figure: WK filters for stable and
stochastic seasonal components).

![**The squared gain of the WK filter for stable and stochastic seasonal
components.**](All_images/UG_A_image11.png)

Since $r\left( \omega \right) \geq 0$, then
$\widetilde{\nu}\left( \omega \right) \leq 1$ and from @eq-29
...
component.[^m-seats-decomposition-25]

[^m-seats-decomposition-25]: Ibid.
...
Indeed, the historical estimator shown in @eq-28<!---\[7.52\]--> is
...
-   $x_{t|T}^{e}$ is the extended series, such that
    $x_{t|T}^{e} = x_{t}$ for $t \leq T$ and
...
$x_{t}$ that generates a phase effect[^m-seats-decomposition-26].

[^m-seats-decomposition-26]: KAISER, R., and MARAVALL, A. (2000).
...
other[^m-seats-decomposition-28]. The revision error decreases as $k$
increases, until it can be assumed equal to 0 for large enough $k$.

[^m-seats-decomposition-28]: MARAVALL, A. (2009).
...
estimator can be expressed as[^m-seats-decomposition-29]:

[^m-seats-decomposition-29]: The section is based on KAISER, R., and
    MARAVALL, A. (2000).
...
$x_{t}$[^m-seats-decomposition-30]. Hence, the filter from @eq-32
...
$\xi_{- 1}F + \ldots + \xi_{- j}F^{j}$ applies to future (i.e. posterior
to $t$) innovations. Consequently, $\xi_{j}$ determines the contribution
of $a_{t - j}$ to ${\widehat{s}}_{t}$ while $\xi_{- j}$ determines the
contribution of $a_{t + j}$ to ${\widehat{s}}_{t}$. Finally, the
estimator of the component can be expressed as:

[^m-seats-decomposition-30]: See section PsiE-weights. For further
    details see MARAVALL, A. (2008).
...
![**WK filters and squared gain of the WK
filter**](All_images/UG_A_image12.png)

It can be shown that ${\xi}_{- 1},\ldots,\xi_{- j}$ are convergent and
$\xi_{j},\ldots, {\xi}_{1},\xi_{0}$ are divergent. From @eq-33
...
preliminary estimators are cointegrated. From expression @eq-25
