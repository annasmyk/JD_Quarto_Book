...
Topics under construction - graphical user interface 3.0 functionalities for high-frequency data
...
Outliers detection is available in the pre-treatment function. Detected outliers are AO, LS and WS. Critical value can be computed by the algorithm or user-defined.
...
-   extreme coefficients $\mathbb{I} \{\text{$E(p)$ pair}\} +(p-E(p)) /2p$
